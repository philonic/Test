...
public class Page<T> {
	...
	private Map<String, Object> queryParams; //查询条件，当页面传递的多个参数不是某个Entity对象属性时适用
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		// 设置排序参数
		String sort = request.getParameter("sort"); //EasyUI 分页参数
		// 设置排序参数
		String order = request.getParameter("order"); //EasyUI 分页参数
		// 设置排序参数
        String orderBy = request.getParameter("orderBy");
        if (StringUtils.isNotBlank(orderBy)) {
            this.setOrderBy(orderBy);
        }else if (StringUtils.isNotBlank(sort) && StringUtils.isNotBlank(order)){
...
//	}
	...
	/**
	 * 查询条件，当页面传递的多个参数不是某个Entity对象属性时适用
	 * @return
	 */
